package net.plazmix.utility.location.region;

import net.plazmix.utility.location.LocationRotator;
...
public class CuboidRegion extends Overlappable {
	...
	public CuboidRegion expand(BlockFace direction, double amount) {
		Vector vec = LocationUtil.getDirection(direction);
	...
	public boolean overlaps(Overlappable overlap) {
		if (!overlap.getWorld().equals(getWorld())) {
			return false;
		}
		if (overlap instanceof MultiRegion) {
			MultiRegion multi = (MultiRegion) overlap;
			return multi.getRegions().stream().anyMatch(r -> overlap.overlaps(this));
		}
		return (!(start.getX() >= overlap.getEnd().getX() || overlap.getStart().getX() >= end.getX()
				|| start.getY() >= overlap.getEnd().getY() || overlap.getStart().getY() >= end.getY()
				|| start.getZ() >= overlap.getEnd().getZ() || overlap.getStart().getZ() >= end.getZ()));
	}
	...
	public CuboidRegion rotate(Location center, int rotations) {
		...
		LocationRotator locationRotator = new LocationRotator(rotations, false);
		locationRotator.setLocation(start.getX(), start.getZ());
		start.setX(locationRotator.getRotatedX());
		start.setZ(locationRotator.getRotatedZ());
		locationRotator.setLocation(end.getX(), end.getZ());
		end.setX(locationRotator.getRotatedX());
		end.setZ(locationRotator.getRotatedZ());
		start.add(center);
		end.add(center);
		setLocations(start, end);
		return this;
