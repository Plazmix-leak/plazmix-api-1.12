...
public class SpheroidRegion extends Region {
	...
	public SpheroidRegion expand(BlockFace face, double amount) {
		...
		move(LocationUtil.getDirection(face).multiply(amount / 2));
		clearCached();
		return this;
	}
	...
	public SpheroidRegion rotate(Location center, int rotations) {
		...
		locationRotator.setLocation(center.getX(), center.getZ());
		center.setX(locationRotator.getRotatedX());
		center.setZ(locationRotator.getRotatedZ());
		if (rotations % 2 == 1) {
			double tmp = xRad;
			xRad = zRad;
			zRad = tmp;
	...
	public String toString() {
		return LocationUtil.locationToString(center) + "-" + xRad + " " + yRad + " " + zRad;
	}
	...
	public static SpheroidRegion fromString(String string) {
		...
		Location center = LocationUtil.stringToLocation(split[0]);
