...
public class MultiRegion extends Overlappable {
	...
	public MultiRegion expand(BlockFace direction, double amount) {
		...
		slice.move(LocationUtil.getDirection(direction));
		for (int i = 0; i < amount; i++) {
			MultiRegion clone = slice.clone();
			clone.move(LocationUtil.getDirection(direction).multiply(i));
			add(clone);
		}
		fixCorners(null);
		return this;
	...
	public void cluster(int per) {
		...
		Location center = Objects.requireNonNull(smallest).getCenter();
	...
	private boolean expandToMax(Region r, MultiRegion exclude, MultiRegionMeta summary) {
		...
		Collections.addAll(faces, LocationUtil.PRIMARY_BLOCK_FACES);
