package net.plazmix.protocollib.team;
...
public class ProtocolTeam {
     ...
     * @param name   - имя тимы
     */
    public static ProtocolTeam find(@NonNull String name) {
     ...
     * @param player - игрок
     */
    public static List<ProtocolTeam> findEntryList(@NonNull Player player) {
        return TEAM_CACHE.values().stream().filter(protocolTeam -> protocolTeam.hasPlayerEntry(player)).collect(Collectors.toList());
    }
     ...
     * @param player - игрок
     */
    public static List<ProtocolTeam> findReceiverList(@NonNull Player player) {
        return TEAM_CACHE.values().stream().filter(protocolTeam -> protocolTeam.hasReceiver(player)).collect(Collectors.toList());
    }
    ...
    private synchronized WrapperPlayServerScoreboardTeam getTeamPacket(int mode) {
        ...
        if (mode == WrapperPlayServerScoreboardTeam.Mode.TEAM_CREATED || mode == WrapperPlayServerScoreboardTeam.Mode.TEAM_UPDATED) {
            ...
            scoreboardTeam.setPrefix(prefix != null ? StringUtils.fixLength(16, prefix) : "");
            scoreboardTeam.setSuffix(suffix != null ? StringUtils.fixLength(16, suffix) : "");

            scoreboardTeam.setPackOptionData(0);
            scoreboardTeam.setColor(prefix != null ? ChatColor.getByChar(prefix.replace(" ", "").charAt(1)).ordinal() : 0);
    ...
    public synchronized void broadcast() {
        for (Player player : getReceiversSet())
            sendPacket(TEAM_UPDATED, player);
    }

    public synchronized void broadcastToServer() {
